				...
				pmUpCht(new ChartDrawer(UpSpeedGbw, StatsPluginSettings::uploadMeasurements()))
{
	...
	pmUpCht -> AddValuesCnt(QPen("#f00"), tr("Current"));
	pmDownCht -> AddValuesCnt(QPen("#f00"), tr("Current"));

	pmUpCht -> AddValuesCnt(QPen("#00f"), tr("Average"));
	pmDownCht -> AddValuesCnt(QPen("#00f"), tr("Average"));
	
	pmUpCht -> AddValuesCnt(tr("Limit"), 0);
	pmDownCht -> AddValuesCnt(tr("Limit"), 0);
	
// 	pmUpCht -> AddValuesCnt(QPen("#f0f"), tr("Current torrent"));
// 	pmDownCht -> AddValuesCnt(QPen("#f0f"), tr("Current torrent"));
// 	
	
	
	pmPeersSpdCht -> AddValuesCnt(QPen("#090"), tr("Average from leecher"));
	pmPeersSpdCht -> AddValuesCnt(QPen("#f00"), tr("Average to leecher"));
	pmPeersSpdCht -> AddValuesCnt(QPen("#00f"), tr("Average from seeder"));
	pmPeersSpdCht -> AddValuesCnt(QPen("magenta"), tr("From leechers"));
	pmPeersSpdCht -> AddValuesCnt(QPen("orange"), tr("From seeders"));
