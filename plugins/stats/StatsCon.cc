				...
				pmDHTCht(new ChartDrawer(DHTGbw, StatsPluginSettings::dHTMeasurements()))
{
	...
	pmPeersConCht -> AddValuesCnt(QPen("#f00"), tr("Leechers connected"));
	pmPeersConCht -> AddValuesCnt(QPen("#900"), tr("Leechers in swarms"));
	pmPeersConCht -> AddValuesCnt(QPen("#00f"), tr("Seeders connected"));
	pmPeersConCht -> AddValuesCnt(QPen("#009"), tr("Seeders in swarms"));
	pmPeersConCht -> AddValuesCnt(QPen("#0a0"), tr("Average connected leechers per torrent"));
	pmPeersConCht -> AddValuesCnt(QPen("#060"), tr("Average connected seeders per torrent"));
 	pmPeersConCht -> AddValuesCnt(QPen("#099"), tr("Average connected leechers per running torrent"));
 	pmPeersConCht -> AddValuesCnt(QPen("#055"), tr("Average connected seeders per running torrent"));

	
	pmDHTCht -> SetUnitName("n");
	
	pmDHTCht -> AddValuesCnt(QPen("#f00"), tr("Nodes"));
	pmDHTCht -> AddValuesCnt(QPen("#00f"), tr("Tasks"));
}
