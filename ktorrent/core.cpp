...
void Core::startTCPServer(bt::Uint16 port)
{
    if (Globals::instance().initTCPServer(port)) {
        ...
        gui->errorMsg(i18n("KTorrent is unable to accept connections because the TCP port %1 is already in use by another program.", port));
...
bool Core::startUTPServer(bt::Uint16 port)
{
    if (Globals::instance().initUTPServer(port)) {
        ...
        gui->errorMsg(i18n("KTorrent is unable to accept connections because the UDP port %1 is already in use by another program.", port));
...
bool Core::init(TorrentControl *tc, const QString &group, const QString &location, bool silently)
{
    ...
    if (qman->checkFileConflicts(tc, conflicting)) {
        ...
        if (!silently) {
            QString err = i18n(
                "Opening the torrent <b>%1</b>, would share one or more files with the following torrents. "
...
{
    Out(SYS_GEN | LOG_IMPORTANT) << "Doing an automatic data check on " << tc->getStats().torrent_name << endl;
